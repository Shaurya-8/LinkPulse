...
export function parseUserAgent(userAgentString: string | null | undefined): ParsedUserAgent & {
  ...
  let device: ParsedUserAgent['device'] = DeviceType.DESKTOP;
  const deviceType = result.device?.type;
  if (deviceType === 'mobile') device = DeviceType.MOBILE;
  else if (deviceType === 'tablet') device = DeviceType.TABLET;
