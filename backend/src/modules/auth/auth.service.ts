...
import { BillingPeriod, PlanType, SubscriptionStatus, UserRole, UserStatus } from "../../../generated/prisma/enums";
...
async function activeSession(user: AuthUser, device: DeviceInfo, tx?: DbClient) {
    ...
    const currSub = user.subscriptions.find((sub) => (sub.status === SubscriptionStatus.ACTIVE && sub.expiresAt! > new Date()));

    const tokens = generateTokenPair(user.id as UserId, user.email, sessionId, user.role, currSub!.plan.name );
...
export async function refreshToken(refreshToken: RefreshToken, device: DeviceInfo) {
     ...
     */
    const { sub, email, sessionId, role, planType } = verifyToken<JwtRefreshPayload>(refreshToken, config.jwt.refreshSecret, 'refresh');
    const oldTokenHash = hashToken(refreshToken);
    await sessionService.getOrRevokeUser(sub, oldTokenHash);

    const tokens = generateTokenPair(sub, email, sessionId, role, planType );
