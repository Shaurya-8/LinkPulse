...
import { AuthenticatedRequest, DeviceInfo, UserId } from "../../types";
import { successResponse } from "../../common/utils/response";
import asyncHandler from "../../common/utils/asyncHandler";
import { CreateLinkInput, GetLinksQuery, RedirectLinkIdInput, UpdateLinkInput } from "./links.schema";
...
export const linksController = {
    ...
    async create(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            const result = await linkService.create(req.validated.body as CreateLinkInput, device, userId);
            successResponse(res,
                result.data,
    ...
    setRedirectRules: asyncHandler(async (req: Request, res: Response) => {
        const userId = (req as AuthenticatedRequest).user.sub;
        const linkId = req.validated.params! as any;
        await linkService.setRedirectRules(linkId.id, userId, req.body.rules);
        successResponse(res, null, 'Redirect rules updated');
    }),

    createAbTest: asyncHandler(async (req: Request, res: Response) => {
        const userId = (req as AuthenticatedRequest).user.sub;

        const test = await linkService.createAbTest(
            (req.params.validated as any).id,
            userId,
            req.body.validated.name,
            req.body.variants,
        );
        successResponse(res, test.data, 'A/B test created');
    }),
