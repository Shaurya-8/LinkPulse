...
export async function sendEmail(options: SendMailOptions): Promise<SentMessageInfo> {
    try {
...
export async function sendOtpEmail(email: string, otp: string, purpose: OtpPurpose, expiresMinutes: number = config.otp.expiresMinutes) {
    const PURPOSE_TEXT_MAP: Record<OtpPurpose, string> = {
