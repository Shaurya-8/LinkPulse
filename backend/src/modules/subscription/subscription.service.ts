...
export class SubscriptionService {
     ...
     */
    async getActive(userId: UserId, tx?: DbClient): Promise<{data: SubscriptionWithPlan}> {
     ...
     */
    async getUserSubscriptions(userId: UserId, tx?: DbClient): Promise<{data: Subscriptions[]}> {
        const subscription = await this.subscriptionRepository.findManyByUserId(userId, tx);
        if (!subscription) throw new SubscriptionError('No Plan Found');
        return {data: subscription};
    }
    /**
    ...
    async getWithFeatureLimitFeatures(userId: UserId, tx?: DbClient): Promise<{data: SubscriptionWithFeatures}> {
        ...
        return {data: subscriptionWithFeature};
    }
    ...
    async changePlan(subscriptionId: SubscriptionId, newPlanId: PlanId, tx?: DbClient): Promise<{data: Subscriptions}> {
     ...
     * Renew subscription
     */
    async renew(subscriptionId: SubscriptionId, tx?: DbClient): Promise<{data: Subscriptions}> {
        ...
        const data = await this.subscriptionRepository.renew(subscriptionId, subscription.expiresAt!);
        return {data};
    }
