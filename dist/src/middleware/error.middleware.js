...
export function sendError(res, status, message, code, extra) {
    const body = {
        success: false,
        message,
        meta: {
...
_next) {
    ...
    if (prismaResult) {
        ...
        sendError(res, prismaResult.status, prismaResult.message, prismaResult.code, { detail: prismaResult.detail });
        return;
    }
    // ── 4. JWT errors ──────────────────────────────────────────────────────────
    ...
    if (err instanceof SyntaxError && 'body' in err) {
        ...
        sendError(res, 400, 'Request body contains invalid JSON. Please check your payload.', 'INVALID_JSON');
        return;
    }
    // ── 6. Payload too large ───────────────────────────────────────────────────
    // Express throws this when the body exceeds the limit set in app.ts (10 KB)
    if (err.type === 'entity.too.large') {
        sendError(res, 413, 'Request payload is too large. Maximum allowed size is 10 KB.', 'PAYLOAD_TOO_LARGE');
        return;
    }
    // ── 7. CORS errors ─────────────────────────────────────────────────────────
    ...
    sendError(res, 500, config.app.isDev
        ? err.message
        : 'An unexpected error occurred. Please try again later.', 'INTERNAL_ERROR', { stack: err.stack });
}
// ─────────────────────────────────────────────────────────────────────────────
// 404 HANDLER
